...
Before building the full product (which involves Cloudflare Workers, durable workflows, Nuxt dashboards, SurrealDB, RabbitMQ, etc.), we need to validate that the **core cryptographic pipeline works at all**. Specifically:
...
If any of these fail or require significant workarounds, we need to know before committing to weeks of infrastructure work.
...
- Designed to surface integration issues, not to be production code
...
If the primary approach (in-circuit ECDSA verification of TLSNotary's attestation) hits blockers, degrade gracefully:

### Fallback Level 1: Two-Step Verification (skip in-circuit ECDSA)

Strip the ECDSA verification from the o1js circuit. Instead:
1. Verify the TLSNotary ECDSA signature off-circuit in a TypeScript script (using `@noble/secp256k1` or similar)
2. Extract the attested fields, compute a Poseidon commitment
3. Have the o1js circuit only verify: Poseidon commitment matches private inputs + eligibility rules pass
4. Post the proof to Zeko

This still proves the ZK eligibility pipeline works and that Zeko settlement works. The attestation verification is done separately — two checks instead of one proof. Acceptable for PoC.

### Fallback Level 2: Skip TLSNotary Entirely (validate o1js + Zeko only)

If TLSNotary integration proves too thorny for the PoC timeframe:
1. Hardcode a secp256k1 keypair
2. Sign a message with it using `@noble/secp256k1` (standard JS library)
3. Verify that signature in an o1js ZkProgram
4. Post the proof to Zeko

This validates that o1js's ECDSA gadget works and Zeko accepts our proofs, without the TLSNotary dependency. TLSNotary integration becomes a separate engineering spike. This approach takes 2-3 days instead of 5-7.
...
**Estimated effort:** 1 developer, 5-7 working days.

| Day | Task | Risk |
|-----|------|------|
| 1 | Set up Rust project. Clone TLSNotary, pin to latest stable release, get their examples compiling. Build the mock HTTPS server. Generate self-signed certs. Verify TLS 1.2 works. | Low |
| 2 | Adapt TLSNotary examples: run Prover + Notary against mock server on localhost. Get the attestation output. Inspect the output format — document the session header structure, signature format, and public key encoding. | **High** — this is where we discover if secp256k1 signing works and what the output looks like |
| 3 | Build the field extractor script. Start the o1js circuit — get basic eligibility checks compiling and proving (salary/tenure/status) **without** ECDSA verification first. This validates o1js fundamentals independently. | Low |
| 4 | Add ECDSA verification to the circuit. Feed real TLSNotary signature data into the o1js ECDSA gadget. Debug format mismatches. This is the most likely failure point. | **High** — signature format compatibility |
| 5 | Deploy the minimal VerificationRegistry zkApp to Zeko testnet. Fund via faucet. Post a proof. Verify the transaction confirms. | **Medium** — Zeko SDK compatibility |
| 6 | Wire everything into `run-poc.sh`. Run end-to-end. Fix issues. Time the pipeline. | Low |
| 7 | Buffer day for debugging. Write the findings document. Update the PoC README with exact setup steps. | — |

**Critical path:** Days 2 and 4. If TLSNotary's secp256k1 output doesn't match o1js's ECDSA input, budget 1-2 extra days for building a conversion layer, or fall back to Level 1 (two-step verification).

---

## 12. Deliverables

1. **Working PoC repository** — `run-poc.sh` completes end-to-end (or, if fallback was needed, the highest-level fallback that works)
2. **Findings document** — concrete answers to all seven "known unknowns" in Section 8, with evidence (logs, timing data, error messages if applicable)
3. **Go/no-go assessment** for the full MVP, structured as:
   - **Confirmed:** what works as documented
   - **Workarounds needed:** what works but requires adaptation (with specific details)
   - **Blockers:** what doesn't work and requires a design change in the MVP architecture
4. **Recommended o1js and TLSNotary versions** to pin for the MVP, based on what actually worked
...
## Appendix B: What This PoC Intentionally Skips

These are all features of the full product that are irrelevant for validating the cryptographic pipeline:

| Skipped Feature | Why |
|-----------------|-----|
| Cloudflare Workers / Workflows | Testing crypto pipeline, not infrastructure |
| Durable orchestration | Sequential CLI script is sufficient |
| SurrealDB / any database | No persistence needed |
| Cloudflare KV | No lookups needed |
| Nuxt dashboard / any UI | No frontend needed |
| RabbitMQ / prover scaling | Single sequential proof is enough |
| Multiple employee records | One hardcoded record validates the pipeline |
| Byte-level selective disclosure | JSON parsing validates the concept |
| Error handling beyond basic | Happy path only |
| Authentication / authorization | None |
| Real HRIS API (Workday/ADP) | Mock is sufficient |
| Hatchet / Temporal | No workflow engine needed |
| Docker / containers | Run everything natively |
